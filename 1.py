...
def filter_contours(contours, min_area=50, max_area=15000):
    ...
    for cnt in contours:
        ...
        #if is_line_like_contour(cnt): continue
        filtered.append(cnt)
    return filtered
...
def contour_to_svg_path(contour):
    ...
    path_data.append("Z")  # Замкнуть контур
...
def process_zone_maps(img1_path, img2_path, output_dir):
    ...
    for i in range(42):
        for cnt in list_cont[i]:
            ...
            if M["m00"] != 0:
                cx = int(M["m10"] / M["m00"])
                cy = int(M["m01"] / M["m00"])
                all1.append((cx, cy, cnt, i))
        for cnt in list_cont_2[i]:
            M = cv2.moments(cnt)
            if M["m00"] != 0:
                cx = int(M["m10"] / M["m00"])
                cy = int(M["m01"] / M["m00"])
                all2.append((cx, cy, cnt, i))

    max_dist = np.sqrt(proc1.shape[0]**2 + proc1.shape[1]**2) * 0.1
    used2 = set()
    change_records = []
    ...
    total_changed = len(change_records)
    total_all = len(all1) + len(all2)
    similarity_percent = 100.0 * (1 - total_changed / total_all) if total_all > 0 else 100.0
    ...
    print(f"[DEBUG] Схожесть: {similarity_percent:.1f}% (изменено {total_changed} из ~{total_all} объектов)")
...
def compare():
    ...
    try:
        ...
        # Генерируем URL через url_for (Flask сам найдёт файл в static/)
